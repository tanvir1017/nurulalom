...
const ReadyToBuildContactForm = () => {
  return (
    <section className="md:container px-4 mb-16 mt-10 md:mt-5">
      <div className="max-w-fit mx-auto text-center space-y-5">
        <Typography
          variant="h2"
          className="text-xl md:text-4xl md:leading-normal leading-[2.2rem]"
        >
          <TextColor>
        ...
        <ul className="flex flex-col md:flex-row  md:items-center items-start md:space-x-5 max-w-fit mx-auto">
          {["20 Hours a month", "Total 4 Months", "2 Lac Taka per month"].map(
            (item: string) => (
              <li key={item} className="md:text-base text-sm">
                <CheckIcon
                  strokeWidth={4.5}
                  className="w-3 h-3 md:w-4 md:h-4 text-blue-500 inline-flex mr-2"
                />
        ...
        <div className="border-t-4 border-blue-500 border  rounded-lg p-5">
                ...
                <List key={labelText}>
                  <ListItem className="p-0 rounded-full">
                    <label
                      ...
                      className="flex w-full cursor-pointer items-center px-3 md:py-2"
                    >
          ...
          <div className="mt-5">
            <Textarea label="ANYTHING ELSE YOU WANT TO ADD?" size="lg" resize />
          </div>
          <div className="mt-5">
